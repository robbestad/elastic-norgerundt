...
function mountVNodeWithBlueprint(node, bp, parentDom, lifecycle, context, instance) {
	...
	if (isTrue$1(bp.hasAttrs)) {
		mountBlueprintAttrs(node, bp, dom, instance);
	}
	if (isTrue$1(bp.hasClassName)) {
		dom.className = node.className;
	}
	if (isTrue$1(bp.hasStyle)) {
		patchStyle(null, node.style, dom);
	}
	if (isTrue$1(bp.hasEvents)) {
...
function renderComponent$1(Component, props, children, context, isRoot) {
	props = addChildrenToProps(children, props);
	...
	if (isStatefulComponent(Component)) {
		...
		instance._pendingSetState = false;
		return renderNode$1(node, context, isRoot);
	} else {
		return renderNode$1(Component(props, context), context, isRoot);
	}
}
	...
	if (children && isArray(children)) {
		...
		for (var i = 0; i < children.length; i++) {
			...
			if (isText || isInvalid) {
				...
				childrenResult.push('<!---->');
				childrenResult.push(renderChildren$1(child));
				childrenResult.push('<!--!-->');
				insertComment = true;
			} else {
				insertComment = false;
				childrenResult.push(renderNode$1(child, context, false));
			}
		}
		...
		if (isStringOrNumber(children)) {
			...
			return renderNode$1(children, context, false) || '';
		}
	}
	return '';
}
...
function renderNode$1(node, context, isRoot) {
	if (!isInvalidNode(node)) {
		var bp = node.bp;
		...
		if (isFunction(tag)) {
			return renderComponent$1(tag, node.attrs, node.children, context, isRoot);
		}
		if (!isNullOrUndefined(className)) {
		...
		if (isRoot) {
			outputAttrs.push('data-sjsroot');
		}
		if (isVoidElement(tag)) {
			return ("<" + tag + (outputAttrs.length > 0 ? ' ' + outputAttrs.join(' ') : '') + ">");
		} else {
			return ("<" + tag + (outputAttrs.length > 0 ? ' ' + outputAttrs.join(' ') : '') + ">" + (html || renderChildren$1(node.children, context)) + "</" + tag + ">");
		}
	}
}

function renderToString(node) {
	return renderNode$1(node, null, false);
}

function renderToStaticMarkup(node) {
	return renderNode$1(node, null, true);
}
