...
describe('Elements (JSX)', () => {
	...
	it('should render a double div and a text node', () => {
		...
		render(<div>{ <div>Hello, Svenjs!</div> }</div>, container);

		expect(container.nodeName).to.equal('DIV');
		expect(container.firstChild.nodeName).to.equal('DIV');
		expect(container.firstChild.textContent).to.equal('Hello, Svenjs!');
	...
	it('should render "className" attribute', () => {
		...
		render(<div className="Svenjs rocks!" />, container);
		expect(container.firstChild.getAttribute('class')).to.eql('Svenjs rocks!');
		expect(
			container.innerHTML
		).to.equal(
			innerHTML('<div class="Svenjs rocks!"></div>')
		);
	});
	...
	it('should render a simple div with dynamic values and props', () => {
		...
		expect(container.childNodes[ 0 ].childNodes[ 0 ].textContent).to.eql('Svenjs');
